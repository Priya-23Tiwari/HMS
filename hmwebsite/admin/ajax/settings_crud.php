<?php  
 ...
 if(isset($_POST['upd_contacts']))
 {
    $frm_data = filteration($_POST);
     ...
     $q = "UPDATE `contact_details` SET `address`=?,`gmap`=?,`pn1`=?,`pn2`=?,`email`=?,`fb`=?,`insta`=?,`tw`=?,`iframe`=? WHERE `sl_no`=?";
 ...
 if(isset($_POST['add_member']))
 {
   ...
   if($img_r == 'inv_img'){
      ...
      $q = "INSERT INTO `team_detailss`(`name`, `picture`) VALUES (?,?)";
 ...
 if(isset($_POST['rem_member']))
 {
   ...
   $pre_q ="SELECT * FROM `team_detailss` WHERE `sr_no`=?";
   $res = select($pre_q,$values,'i');
   $img = mysqli_fetch_assoc($res);

   if(deleteImage($img['picture'],ABOUT_FOLDER)){
     $q = "DELETE FROM `team_detailss` WHERE `sr_no`=?";
